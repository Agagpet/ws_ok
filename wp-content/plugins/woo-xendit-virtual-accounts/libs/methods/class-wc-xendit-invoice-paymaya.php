...
class WC_Xendit_Paymaya extends WC_Xendit_Invoice {
    ...
    const DEFAULT_PAYMENT_DESCRIPTION = 'Pay with your PayMaya via <strong>Xendit</strong>';
    ...
    public function __construct() {
        ...
        $this->method_title = __('Xendit PayMaya', 'woocommerce-gateway-xendit');
        $this->method_description = sprintf(__('Collect payment from PayMaya on checkout page and get the report realtime on your Xendit Dashboard. <a href="%1$s" target="_blank">Sign In</a> or <a href="%2$s" target="_blank">sign up</a> on Xendit and integrate with <a href="%3$s" target="_blank">your Xendit keys</a>.', 'woocommerce-gateway-xendit' ), 'https://dashboard.xendit.co/auth/login', 'https://dashboard.xendit.co/register', 'https://dashboard.xendit.co/settings/developers#api-keys');
    ...
    public function get_icon() {
        $style = version_compare(WC()->version, '2.6', '>=') ? 'style="margin-left: 0.3em; max-height: 28px; max-width: 90px;"' : '';
        $icon = '<img src="' . plugins_url('assets/images/paymaya.png', WC_XENDIT_PG_MAIN_FILE) . '" alt="Xendit" ' . $style . ' />';
