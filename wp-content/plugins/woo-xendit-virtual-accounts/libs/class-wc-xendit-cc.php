    ...
    */
    ...
    public function set_subscription_items()
    {
        foreach (WC()->cart->get_cart() as $cart_item) {
            if ($cart_item['data']->get_type() == 'subscription') {
    ...
    public function set_current_currency($currency = 'IDR')
    {
        switch ($currency) {
            case 'IDR':
                $this->DEFAULT_MINIMUM_AMOUNT = 5000;
                $this->DEFAULT_MAXIMUM_AMOUNT = 200000000;
                break;

            case 'PHP':
                $this->DEFAULT_MINIMUM_AMOUNT = 20;
                $this->DEFAULT_MAXIMUM_AMOUNT = 700000;
                break;
    ...
    public function get_icon()
    {
        ...
        if (!empty($cc_settings['supported_card_brands'])) {
            ...
            $icon = '<img src="' . plugins_url('assets/images/cc-default.png', WC_XENDIT_PG_MAIN_FILE) . '" alt="Xendit" style="margin-left:0.3em; max-height:28px; max-width:90px;" />';
        }
     ...
     * @return array()
     */
    function generate_payment_request($order, $xendit_token, $auth_id = '', $duplicated = false, $is_recurring = false, $check_ccpromo = true)
    {
        global $woocommerce;
     ...
     * @param WC_Order $order
     */
    private function process_payment_via_xendit_invoice($order) {
