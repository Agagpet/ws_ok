...
final class WC_Xendit_PG_Helper {
    ...
    public static function generate_items_and_customer($order, $total_amount, $type = '') {
        ...
        foreach ($order->get_items() AS $item_data) {           
            ...
            $item['price']      = $product->get_price() ? $product->get_price() : 0;
            $item['type']       = "PRODUCT";
            $item['quantity']   = $item_data->get_quantity();
            $item['url']        = get_permalink($item['id']);
            $item['category']   = $category_name ? $category_name : 'Uncategorized';
        ...
        $customer['mobile_number']          = $order->get_billing_phone() ? $order->get_billing_phone() : 'N/A';
